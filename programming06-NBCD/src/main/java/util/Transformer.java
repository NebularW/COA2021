...
public class Transformer {
    ...
    public String intToBinary(String numStr) {
        ...
        if (num == 0) return "00000000000000000000000000000000";  //0单独判断
        ...
        String ans = temp.reverse().toString();  //反转，tmp是倒的
        int len = ans.length();
    ...
    private int valueOf(String num, int radix) {
        ...
        for (int i = 0; i < num.length(); i++) {
            ...
            else temp = num.charAt(i) - 'a' + 10;//16进制
            ans = ans * radix + temp;
        }
        return ans;
    }
...
//		return result;  //用于测试最大值
    ...
    private String integerRepresentation(String number, int length) {
        ...
        if (num < 0) return Integer.toBinaryString(num).substring(32 - length);
            ...
            int len = length - result.length();  //这一步要先提取出来，不然下面会反复计算len
            for (int i = 0; i < len; i++) {
                result = "0" + result;
            }
            return result;
