...
public class ALU {
    ...
    public String mul_56(String src, String dest) {
        int length = 28;  // length为数据长度
        String neg = oneAdder(negation(src)).substring(1);  //取反加一,去掉第一位溢出位
        String product = "0000000000000000000000000000" + dest;
        int Y1 = 0;
        int Y2 = product.charAt(2 * length - 1) - '0';
        for (int i = 0; i < length; i++) {
            switch (Y1 - Y2) {
                case 1:
                    product = adder(product.substring(0, length), src, '0', length) + product.substring(length);
                    break;
                case -1:
                    product = adder(product.substring(0, length), neg, '0', length) + product.substring(length);
                    break;
            }
            product = product.substring(0, 1) + product.substring(0, product.length() - 1);  //算数右移
            Y1 = Y2;  //更新两个Y
    ...
    public String div_f(String srcStr, String destStr) {
        int length = 54;
        String quotient = "";
        String remainder = "";
        boolean op1Zero = isZero(destStr);
        ...
        if (isZero(adder(srcStr, impleDigits(remainder, length / 2), '0', length / 2))) {
            quotient = add(new DataType("00000" + impleDigits(quotient, length / 2)), new DataType(BinaryIntegers.NegativeOne)).toString().substring(5);
            remainder = BinaryIntegers.ZERO;
            remainderReg = new DataType(remainder);
            return quotient;
        } else if (remainder.equals(srcStr)) {
        ...
        //溢出情况前面判断了，补齐位数
        //remainderReg = new DataType(impleDigits(remainder, length / 2));
        return impleDigits(quotient, length / 2);
    }
