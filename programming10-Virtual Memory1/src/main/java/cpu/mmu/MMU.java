...
public class MMU {
     ...
     * 实模式下的逻辑地址转线性地址
     *
     * @param logicAddr 48位 = 16位段寄存器 + 32位offset，
     *                  计算公式为：①(16-bits段寄存器左移4位 + offset的低16-bits) = 20-bits物理地址 ②高位补0到32-bits
     * @return 32-bits实模式线性地址
     */
    private String toRealLinearAddr(String logicAddr) {
        int base = Integer.parseInt(transformer.binaryToInt("0" + logicAddr.substring(0, 16)));
        int offset = Integer.parseInt(transformer.binaryToInt("0" + logicAddr.substring(16, 48)));
        return "000000000000" + transformer.intToBinary(String.valueOf((base << 4) + offset)).substring(12);
    }
    ...
    private String toSegLinearAddr(String logicAddr) {
        ...
        int base = Integer.parseInt(transformer.binaryToInt(String.valueOf(memory.getBaseOfSegDes(index))));
        int offset = Integer.parseInt(transformer.binaryToInt("0" + logicAddr.substring(16, 48)));
        return transformer.intToBinary(String.valueOf(base + offset));
    }
    ...
    private String toPagePhysicalAddr(String linearAddr) {
        ...
        int index = Integer.parseInt(transformer.binaryToInt("000000000000" + String.valueOf(memory.getFrameOfPage(vPageNo))));//32位物理地址
        String base = transformer.intToBinary(String.valueOf(index)).substring(12, 32);
        return base + linearAddr.substring(20, 32);
    }
